...
\chapter{Analisi}
Il software ha lo scopo di replicare in una formula rivisitata lo storico videogame arcade NEO Bomberman.
Il player, nei panni del Bomberman, sarà catapultato in un'arena situata all'interno di un castello.
\\
L'obbiettivo del protagonista sarà quello di distruggere tutte le casse presenti nella mappa entro lo scadere di un certo limite di tempo. 
\\
\textbf{ATTENZIONE!} Ad ostacolare l'impresa del Bomberman saranno presenti pericolosi nemici, eliminali tutti!!

\section{Requisiti}

\subsubsection{Requisiti funzionali}
\begin{itemize}
    \item Il menù di gioco, visualizzato all'avvio dell'applicazione, permette di iniziare una nuova partita (selezionando la difficoltà) e di accedere alla classifica.
	\item Una volta lanciata la partita, verrà creata una mappa 2D nel quale saranno presenti numerose entità, quali Protagonista Casse e Nemici.
	\item La skin del personaggio dovrà essere modificabile.
	\item Il Bomberman avrà a disposizione 3 vite.
	\item Il protagonista potrà tentare di distruggere le altre entità tramite il rilascio di bombe, accumulando mano a mano Punti abilità.
	\item I nemici saranno in grado di colpire il protagonista corpo a corpo, togliendogli una vita.
	\\
	\item Le casse colpite potrebbero rilasciare un powerUp tra i seguenti: \begin{itemize}
	    \item \textbf{TimeExtra}: saranno aggiunti secondi al tempo rimanente.
	    \item \textbf{Pierce}: l'esplosione delle bombe potrà trapassare casse e nemici.
	    \item \textbf{FirePower}: l'esplosione delle bombe potrà colpire più lontano.
	    \item \textbf{LifeExtra}: verrà aggiunta una vita al Bomberman.
	    \item \textbf{Speed}: la velocità del Bomberman verrà aumentata.
	\end{itemize}
	\item La partita termina quando il Bomberman avrà terminato le vite, oppure quando sarà scaduto il tempo, oppure ancora quando si saranno eliminati tutti i nemici e tutte le casse.
	\item Una volta conclusa la partita, il nome del player verrà salvato nella classifica insieme al relativo punteggio.
\end{itemize}
