...
\author{Francesco Agostinelli \\ Tommaso Brini \\ Luigi Borriello \\ Gustavo Mazzanti}
\date{\today}
...
\chapter{Analisi}
Il software ha lo scopo di replicare in una formula rivisitata lo storico videogame arcade NEO Bomberman (\url{https://it.wikipedia.org/wiki/Neo_Bomberman}).
Il player, nei panni del Bomberman avrà l'obbiettivo di distruggere tutte le casse presenti nella mappa entro lo scadere di un certo limite di tempo. 
\\
Ad ostacolare il Bomberman saranno presenti dei nemici, anch'essi da eliminare.

\section{Requisiti}

\subsubsection{Requisiti funzionali}
\begin{itemize}
	\item Creazione di una mappa 2D nel quale saranno presenti svariate entità, con le quali il protagonista potrà interagire.
	\item Implementazione di due diverse difficoltà, che andranno a modificare l'esperienza di gioco:
	\begin{itemize}
	    \item \textbf{Difficoltà Standard}: 
	    \item \textbf{Difficoltà Hardcore}: 
	\end{itemize}
	    
	\item Realizzazione del Bomberman in grado di spostarsi all'interno della mappa e di rilasciare le bombe.
	\item Realizzazione di nemici dotati di intelligenza, e in grado di colpire corpo a corpo il Player.
	\item Realizzazione di svariati PowerUp (potenziamenti per il Bomberman).
	\item Creazione di elementi distruttibili, nonchè le casse di gioco.
	\item Realizzazione del respawn dei nemici.
	\item Fornire all'utente la possiblità di salvare il proprio score, in una classifica globale.
	\item Realizzazione di un menu nel quale l'utente potrà navigare e scegliere se lanciare una partita, oppure accedere alla classifica dei punteggi.
	\\
...
\begin{itemize}
	\item Prestazioni in grado di fornire una buona esperienza di gioco.
	\item Disposizione casuale delle casse all'interno della mappa.
	\item Possibilità da parte dell'utente di scegliere i comandi di gioco e l'aspetto del Bomberman.
\end{itemize}


\section{Analisi e modello del dominio}

Una volta avviata la partita il giocatore verrà collocato all'interno della mappa nella quale saranno presenti casse, nemici e muri indistruttibili.
Il Bomberman per vincere la partita dovrà aver distrutto, tramite il rilascio di bombe, tutte le casse e tutti i nemici presenti, entro lo scadere del tempo limite.
Le difficoltà primarie saranno:
\begin{itemize}
    \item Far interagire il protagonista con i nemici e l'ambiente di gioco gestendo al meglio le varie collisioni e i vari eventi.
    \item Realizzare due difficoltà del gameplay caratterizzate da dinamiche differenti.
\end{itemize}
\begin{figure}
    \centering
    \includegraphics[scale=0.8]{BomberOneReport/img/uml.png}
    \caption{Schema UML dell’analisi dei requisiti indicante le principali entità e relazioni che comporranno l’applicazione}
